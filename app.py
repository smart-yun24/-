...
st.markdown("""
    ...
    .abandoned-card-보전 { background-color: #f0f7ff !important; border-left: 5px solid #2563eb; }
    .abandoned-card-처분 { background-color: #fff7ed !important; border-left: 5px solid #ea580c; }
...
def display_cards(res_df, mode="river"):
    for _, row in res_df.head(30).iterrows():
        ...
        if mode == "river":
            ...
            plan_html = f'<span style="font-size:0.75rem; font-weight:800; border-bottom:2px solid currentColor;">({row["계획"]})</span>'

        st.markdown(f"""
            <div class="property-card {card_cls}">
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <span class="address-text">📍 {full_addr} {plan_html}</span>
                ...
                <div class="btn-grid">
                    ...
                    <a href="https://www.eum.go.kr/web/am/amMain.jsp?searchType=address&query={encoded_addr}" target="_blank" class="eum-btn">토지이용확인(이음)</a>
                </div>
            </div>
        """, unsafe_allow_html=True)
