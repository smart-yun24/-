...
# 2. UI 디자인 (폰트 사이즈 및 대시보드 스타일 정밀 조정)
st.markdown("""
    <style>
    ...
    .metric-card {
        flex: 1; min-width: 150px; background-color: white; padding: 15px; border-radius: 12px;
    ...
    if not summary_df.empty:
        ...
        st.markdown(f"""
            <div class="metric-container">
                <div class="metric-card"><div class="metric-label">총 필지 수</div><div class="metric-value">{summary_df["필지수"].sum():,}건</div></div>
                <div class="metric-card"><div class="metric-label">총 지적면적 합계</div><div class="metric-value">{summary_df["지적합계(㎡)"].sum():,.0f}㎡</div></div>
                <div class="metric-card"><div class="metric-label">총 편입면적 합계</div><div class="metric-value" style="color:#ef4444;">{summary_df["편입합계(㎡)"].sum():,.0f}㎡</div></div>
                <div class="metric-card"><div class="metric-label">국유지 비율</div><div class="metric-value">{(summary_df["국유지"].sum()/summary_df["필지수"].sum()*100):.1f}%</div></div>
            </div>
        """, unsafe_allow_html=True)
        
        st.write("")
        st.markdown("**📍 지역별 상세 요약 현황**")
        st.dataframe(summary_df.style.format({
            "필지수": "{:,}", "국유지": "{:,}", "사유지": "{:,}", "지적합계(㎡)": "{:,.0f}", "편입합계(㎡)": "{:,.0f}"
        }), use_container_width=True, hide_index=True)
    else:
        st.info("데이터 파일을 GitHub에 업로드하면 이곳에 집계 결과가 나타납니다.")
...
with tab1:
    with st.popover("🔍 지역 및 지번 검색"):
        ...
        else: st.warning(f"'{sel_reg}' 파일이 없습니다.")
...
# --- [Tab 2: 폐천부지 조회] ---
with tab2:
    st.info("폐천부지 엑셀 양식을 업로드해주시면 이곳에 맞춤형 화면이 구성됩니다.")