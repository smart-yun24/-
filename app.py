...
with tab1:
    with st.popover("지역 및 지번 검색"):
        ...
        if df is not None:
            dong = st.selectbox("동/리", options=["전체"] + sorted(df['동리'].dropna().unique().astype(str).tolist()), key="r_dong")
...
with tab2:
    ...
    with f_c2: check_cheobun = st.checkbox("처분", value=True, key="f_cheobun")
    ...
    with st.popover("상세 지역 검색"):
        ...
        if df_d_all is not None:
            ...
            dong_d = st.selectbox("동/리 ", options=["전체"] + sorted(df_d['동리'].dropna().unique().astype(str).tolist()), key="d_dong")
    ...
    if df_d is not None:
        ...
        st.markdown(f"**조회 결과: {len(res_d):,}건**")
...
with tab3:
    with st.popover("상세 지역 검색"):
        ...
        if df_f_all is not None:
            ...
            dong_f = st.selectbox("동/리  ", options=["전체"] + sorted(df_f['동리'].dropna().unique().astype(str).tolist()), key="f_dong")
    ...
    if df_f is not None:
        ...
        st.markdown(f"**조회 결과: {len(res_f):,}건**")
