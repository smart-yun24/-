...
# 1. 페이지 설정 및 모바일 최적화 (초슬림 모드)
st.set_page_config(page_title="하천구역 스마트 조회", layout="wide", initial_sidebar_state="collapsed")

# 2. 가독성을 위한 20% 축소 폰트 및 지목 배경색 CSS
st.markdown("""
    <style>
    .stApp { background-color: #f8fafc; }
    html, body { font-size: 0.85rem; } /* 전체 폰트 20% 축소 반영 */
    ...
    /* 주소 및 소유자 헤더 */
    .card-header-box { display: flex; flex-direction: column; margin-bottom: 10px; }
...
with st.popover("🔍 지역 및 지번 검색"):
    ...
    if os.path.exists(file_path):
        # 엑셀 시트 1개를 읽어옴 (2행이 제목)
        df_raw = pd.read_excel(file_path, sheet_name=0, header=1)
        
        # [무적 로직] 열이 몇 개든 앞에서부터 10개만 딱 잘라서 씁니다
...
for _, row in filtered_df.head(50).iterrows():
    ...
    # HTML 카드 렌더링 (unsafe_allow_html=True 필수!)
    st.markdown(f"""
