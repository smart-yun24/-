...
import pandas as pd
import os
import urllib.parse  # 주소 인코딩 처리를 위해 필수

# 1. 페이지 설정 및 전문가용 폰트 설정
st.set_page_config(page_title="낙동강 상류 조서 조회 서비스", layout="wide", initial_sidebar_state="collapsed")

# 2. 유신 시그니처 테마 (Deep Navy & Glassmorphism 디자인)
st.markdown("""
    <style>
    /* 전체 배경: 깔끔한 오프화이트 */
    ...
    .map-btn { 
        ...
        padding: 12px; border-radius: 10px; text-decoration: none !important; font-weight: 800; font-size: 0.9rem;
    }
    .eum-btn { 
    ...
    if not sum_df.empty:
        ...
        st.markdown(f"""
            <div class="metric-container">
                ...
                <div class="metric-card"><div class="metric-label">편입면적 합계</div><div class="metric-value" style="color:#dc2626;">{sum_df["편입면적 합계(㎡)"].sum():,.0f}㎡</div></div>
            </div>
        """, unsafe_allow_html=True)
...
# --- [카드 출력 공통 함수] ---
def display_land_cards(res_df, mode="river"):
    for _, row in res_df.head(30).iterrows():
        # [수정] 읍면 데이터를 포함한 정밀 주소 생성
        addr_list = [str(row['시군']), str(row['읍면']), str(row['동리']), str(row['번지'])]
        full_addr = " ".join([p.strip() for p in addr_list if p and str(p).lower() != 'nan'])
        
        # URL 안전 인코딩 (공백을 +로 처리하여 토지이음 호환성 강화)
        ...
        if mode == "river":
            ...
            card_cls = f"abandoned-card-{plan_val}" if plan_val in ["보전", "처분"] else "property-card"
            plan_label = f'<span style="font-size:0.75rem; font-weight:800; border-bottom:2px solid currentColor;">({plan_val})</span>'
...
with tab2:
    ...
    if df_d is not None:
        ...
        display_land_cards(res_d, "delete")